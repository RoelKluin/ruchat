...
impl Validation {
    ...
    /// Alternative to `apply_patch` for a single, localized change: instead of a unified diff
    /// (line numbers, hunk-count bookkeeping, `+`/`-`/` ` prefixes — the exact formatting
    /// coder-tuned local models reliably get subtly wrong, see `fix_hunk_header_counts`/
    /// `normalize_diff_hunk_lines` above, both permanent repairs for classes of malformed diff
    /// this tool sidesteps entirely), the model gives an exact existing snippet (`old_string`)
    /// and its replacement (`new_string`) — no line-count math, no diff syntax to get subtly
    /// wrong. `old_string` must match exactly once in the file: zero matches means it doesn't
    /// reflect the file's real content (same failure mode `apply_patch`'s context mismatch
    /// handles, and the same fix — show the real content rather than costing another round
    /// trip); more than one match is ambiguous about which occurrence is meant, so it's refused
    /// rather than guessed at (matching the same "resolve unambiguously or refuse" posture
    /// `apply_patch` takes for a missing file header).
    pub(crate) async fn replace_in_file(
        ...
        if old_string.is_empty() {
            let content = "refused: 'old_string' must not be empty — replace_in_file finds \
                and replaces an exact existing snippet, it can't match nothing. Use \
                apply_patch if you need to create a brand new file."
                .to_string();
            ctx.push_turn(TurnKind::Rejection, "Validator", content.clone());
            return Ok(Validation::Failure(content));
        }
        if old_string == new_string {
            let content =
                "refused: 'old_string' and 'new_string' are identical — this would change \
                nothing."
                    .to_string();
            ctx.push_turn(TurnKind::Rejection, "Validator", content.clone());
            return Ok(Validation::Failure(content));
        }
        let combined_len = old_string.len() + new_string.len();
        if combined_len > MAX_PATCH_DIFF_BYTES {
            let content = format!(
                "refused: combined old_string+new_string is {combined_len} bytes, exceeds the \
                {MAX_PATCH_DIFF_BYTES}-byte limit for a single replace_in_file call — split \
                this into smaller, more focused replacements."
            );
            ctx.push_turn(TurnKind::Rejection, "Validator", content.clone());
            return Ok(Validation::Failure(content));
        }
        ...
        if !tracked.contains(path) {
            let content = format!(
                "refused: '{path}' is not tracked by git (not in `git ls-files`) — \
                replace_in_file may only modify files already under version control in this \
                repo"
            );
            ctx.push_turn(TurnKind::Rejection, "Validator", content.clone());
            return Ok(Validation::Failure(content));
        }
        // Same scope enforcement as `apply_patch` — see its own comment on this check.
        let planned = ctx.planned_files();
        if !planned.is_empty() && !file_in_scope(path, &planned) {
            let content = format!(
                "refused: '{path}' is not one of the files the plan named with its `FILES:` \
                line ({}) — replace_in_file may only touch files the Architect's plan \
                declared in scope for this round",
                planned.join(", ")
            );
            ctx.push_turn(TurnKind::Rejection, "Validator", content.clone());
            return Ok(Validation::Failure(content));
        ...
        if match_count == 0 {
            ...
            let content = format!(
                "replace_in_file failed on {path}: 'old_string' was not found anywhere in the \
                file. This means old_string doesn't match {path}'s actual current content. \
                Here is the file's real current content — write your next old_string to match \
                this exactly, don't guess:\n\n{shown}{truncated_note}"
            );
            ctx.push_turn(TurnKind::Rejection, "Validator", content);
            return Ok(Validation::Failure(format!("no match for old_string in {path}")));
        }
        if match_count > 1 {
            let content = format!(
                "replace_in_file failed on {path}: 'old_string' matches {match_count} \
                separate locations in the file, but replace_in_file requires it to match \
                exactly one — otherwise it's ambiguous which occurrence you mean. Include \
                more surrounding context (e.g. a preceding comment or the enclosing function \
                signature) in old_string to make it unique, then resubmit."
            );
            ctx.push_turn(TurnKind::Rejection, "Validator", content.clone());
            return Ok(Validation::Failure(content));
        }
        ...
        // Same revert bookkeeping as `apply_patch` — see `Context::record_patch`.
        ctx.record_patch(path.to_string(), original);
        Ok(Validation::Success)
    }
...
mod tests {
    ...
    // `replace_in_file` tests below deliberately only exercise rejection paths (matching the
    // `apply_patch` tests above) — every one of them returns before ever writing to disk, so
    // none mutates this repo's own tracked files during `cargo test`. There's no test for the
    // successful-write path for the same reason `apply_patch` doesn't have one either: it would
    // require either a real write against this repo's actual working directory or extending
    // the function to accept an injectable path, both out of scope here.

    #[tokio::test]
    async fn replace_in_file_rejects_an_empty_old_string() {
        let mut ctx = Context::new("goal".to_string());
        let result = Validation::replace_in_file("Cargo.toml", "", "new", &mut ctx)
            .await
            .unwrap();
        match result {
            Validation::Failure(msg) => assert!(msg.contains("must not be empty")),
            other => panic!("expected rejection, got: {other:?}"),
        }
    }

    #[tokio::test]
    async fn replace_in_file_rejects_identical_old_and_new_string() {
        let mut ctx = Context::new("goal".to_string());
        let result = Validation::replace_in_file("Cargo.toml", "same", "same", &mut ctx)
            .await
            .unwrap();
        match result {
            Validation::Failure(msg) => assert!(msg.contains("identical")),
            other => panic!("expected rejection, got: {other:?}"),
        }
    }

    #[tokio::test]
    async fn replace_in_file_rejects_a_combined_size_over_the_byte_limit() {
        let mut ctx = Context::new("goal".to_string());
        let huge_old = "a".repeat(MAX_PATCH_DIFF_BYTES);
        let result = Validation::replace_in_file("Cargo.toml", &huge_old, "b", &mut ctx)
            .await
            .unwrap();
        match result {
            Validation::Failure(msg) => assert!(msg.contains("exceeds the")),
            other => panic!("expected rejection, got: {other:?}"),
        }
    }

    #[tokio::test]
    async fn replace_in_file_rejects_a_file_not_tracked_by_git() {
        let mut ctx = Context::new("goal".to_string());
        let result =
            Validation::replace_in_file("definitely_not_a_real_tracked_file.rs", "x", "y", &mut ctx)
                .await
                .unwrap();
        match result {
            Validation::Failure(msg) => assert!(msg.contains("not tracked by git")),
            other => panic!("expected rejection, got: {other:?}"),
        }
    }

    #[tokio::test]
    async fn replace_in_file_rejects_target_outside_declared_scope() {
        let mut ctx = Context::new("goal".to_string());
        ctx.push_turn(
            TurnKind::Plan,
            "Architect",
        ...
        let result = Validation::replace_in_file("Cargo.toml", "x", "y", &mut ctx)
            .await
            .unwrap();
        match result {
            Validation::Failure(msg) => assert!(msg.contains("not one of the files")),
            other => panic!("expected rejection, got: {other:?}"),
        }
    }

    #[tokio::test]
    async fn replace_in_file_shows_real_file_content_when_old_string_is_not_found() {
        let mut ctx = Context::new("goal".to_string());
        let result = Validation::replace_in_file(
            "Cargo.toml",
            "this exact string definitely does not appear in Cargo.toml",
            "replacement",
            &mut ctx,
        )
        .await
        ...
        match result {
            Validation::Failure(_) => {
                let rejection = ctx
                    ...
                    .iter()
                    .find(|t| t.kind == TurnKind::Rejection)
                    .expect("a failed replace should push a rejection");
                assert!(rejection.content.contains("real current content"));
                assert!(
                    rejection.content.contains("[package]"),
                    "expected Cargo.toml's real content in the message, got: {}",
            ...
            other => panic!("expected the replace to fail, got: {other:?}"),
        }
    }

    #[tokio::test]
    async fn replace_in_file_rejects_an_old_string_matching_more_than_once() {
        let mut ctx = Context::new("goal".to_string());
        // "=" appears many times in Cargo.toml (every `key = value` line) — genuinely ambiguous.
        let result = Validation::replace_in_file("Cargo.toml", "=", "==", &mut ctx)
            .await
            .unwrap();
        match result {
            Validation::Failure(msg) => assert!(
                msg.contains("separate locations") && msg.contains("exactly one"),
                "expected an ambiguous-match rejection, got: {msg}"
            ),
            other => panic!("expected rejection, got: {other:?}"),
        }
    }
