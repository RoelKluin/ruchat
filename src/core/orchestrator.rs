    ...
    async fn execute_orchestration(
        ...
        for round in 1..=iterations {
                ...
                && let Some(librarian) = self.librarian.as_mut()
            {
                ...
                ctx.trace(
                    &tx,
                    format!("Librarian formulated query: {}", ctx.output.as_str()),
                )
                .await;
                ...
                ctx.trace(
                    &tx,
                    format!(
                        "✅ Librarian found {} results. Documents now in context for Worker.",
                        num_docs
                    ),
                )
                .await;
            ...
            if let Some(validator) = self.validator.as_mut() {
                ...
                if ctx.output.contains("REJECTED") {
                    ctx.trace(
                        &tx,
                        format!("Round {round} rejected by validator: {}", ctx.output),
                    )
                    .await;
                    continue;
                }
        ...
        for role in sequence {
            let agent = match role.as_str() {
                ...
                "Librarian" => self
                    .librarian
                    .as_mut()
                    .ok_or(RuChatError::Is("Librarian not enabled".into()))?,
                "Validator" => self
                    .validator
                    .as_mut()
