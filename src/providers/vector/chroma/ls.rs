...
impl ChromaLsArgs {
    pub(crate) async fn ls(&self) -> Result<()> {
        ...
        let collections = if self.collection.name().is_empty() {
            client.list_collections(100, None).await.map_err(RuChatError::ChromaHttpClientError)?
        } else {
        ...
        for col in collections {
            let count = col.count().await.map_err(RuChatError::ChromaHttpClientError)?;

            let (schema, indexing_status) = if self.long {
                (
                    col.schema().clone(),
                    Some(col.get_indexing_status().await.map_err(RuChatError::ChromaHttpClientError)?)
                )
            } else {
                (None, None)
            };
    ...
    fn render(&self, data: Vec<CollectionInfo>) -> Result<()> {
        if self.json {
            println!("{}", serde_json::to_string_pretty(&data).map_err(|e| RuChatError::InternalError(e.to_string()))?);
        } else {
            if self.long {
                for item in data {
                ...
                // Compact Table View
                println!("{:<20} {:<38} {:<10} {:<10}", "NAME", "ID", "COUNT", "VERSION");
                for item in data {
                    println!("{:<20} {:<38} {:<10} {:<10}",
