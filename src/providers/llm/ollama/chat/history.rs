...
impl EditKind {
    pub(crate) fn apply(&self, lines: &mut Vec<String>, before: &Pos, after: &Pos) -> ClearType {
        match self {
...
impl History {
    ...
    pub fn redo(&mut self, lines: &mut Vec<String>) -> Option<((usize, usize), ClearType)> {
        if self.index >= self.edits.len() {
            return None;
        }
        let edit = &self.edits[self.index];
        let res = edit.redo(lines);
        self.index += 1;
        Some((edit.cursor_after(), res))
    }

    pub fn undo(&mut self, lines: &mut Vec<String>) -> Option<((usize, usize), ClearType)> {
        self.index = self.index.checked_sub(1)?;
        let edit = &self.edits[self.index];
        let res = edit.undo(lines);
        Some((edit.cursor_before(), res))
    }
