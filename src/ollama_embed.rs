...
pub(crate) async fn embed(
    ...
    if let Some(md) = &embed_args.metadata {
        for s in md.split(',') {
            match s.split_once(':') {
    ...
    let request =
        GenerateEmbeddingsRequest::new(model_name, vec![embed_args.prompt.as_str()].into());
    let client = create_chroma_client(&args).await?;
    let res = ollama.generate_embeddings(request).await?;

    let collection: ChromaCollection = client
        .get_or_create_collection(&embed_args.collection, None)
        .await?;
    let count_str = collection.count().await?.to_string();

    let collection_entries = CollectionEntries {
        ids: vec![count_str.as_str().into()],
        embeddings: Some(res.embeddings),
        metadatas: Some(vec![metadata]),
        documents: Some(vec![&embed_args.prompt]),
    };
